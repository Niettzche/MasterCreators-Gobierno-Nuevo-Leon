    ...
    <div className="-m-8 p-8 min-h-full w-full">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Gestione sus solicitudes y seguimientos.</p>
        </div>
        <button 
          onClick={() => navigate('/register-project')}
          className="bg-orange-600 hover:bg-orange-700 text-white px-5 py-2.5 rounded-xl font-bold shadow-lg shadow-orange-500/20 transition-all flex items-center gap-2"
        >
        ...
        {projects.map((project, idx) => (
          ...
          >
            ...
            <div className={`absolute top-0 left-0 w-1.5 h-full ${
              ...
              project.color === 'emerald' ? 'bg-emerald-500' : 'bg-slate-300'
            }`}></div>
            ...
            <div className="pl-2 mb-6">
              <h3 className="text-lg font-bold text-slate-800 leading-tight mb-1 group-hover:text-orange-600 transition-colors cursor-pointer">
                {project.title}
              </h3>
              <p className="text-xs text-slate-500 uppercase tracking-wide font-medium">{project.program}</p>
            </div>

            <div className="pl-2 space-y-3 border-t border-slate-50 pt-4">
              <div className="flex justify-between text-sm">
                <span className="text-slate-500">Monto Solicitado</span>
                <span className="font-bold text-slate-800">${project.amount.toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-500">Fecha Envío</span>
                <span className="font-medium text-slate-700">{project.submissionDate}</span>
              </div>
            </div>
            ...
            <div className="pl-2 mt-6 flex gap-2">
              ...
              {project.status === 'Borrador' && (
                <button className="flex-1 bg-orange-50 text-orange-600 border border-orange-100 text-xs font-bold py-2 rounded-lg hover:bg-orange-100 transition-colors">
                  Continuar
                </button>
              )}
            </div>
