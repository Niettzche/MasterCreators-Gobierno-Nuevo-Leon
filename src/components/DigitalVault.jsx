  ...
  return (
    <div className="-m-8 p-8 min-h-full w-full font-sans text-slate-900">
      ...
      <header className="flex justify-between items-end mb-8">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Bóveda Digital</h1>
          <p className="text-slate-500 text-sm mt-1 font-medium">Repositorio seguro de documentación legal y fiscal.</p>
        </div>
        <button className="bg-orange-600 hover:bg-orange-700 text-white px-5 py-2.5 rounded-xl font-bold text-sm shadow-lg shadow-orange-500/20 flex items-center gap-2 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
            ...
            <div>
                ...
                <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">Documentos Totales</p>
            </div>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
            ...
            <div>
                ...
                <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">Vigencia Global</p>
            </div>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
            ...
            <div>
                ...
                <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">Atención Requerida</p>
            </div>
        </div>
      </div>
        ...
        <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            ...
            <div className="relative">
                <input 
                    ...
                    className="pl-9 pr-4 py-2 rounded-lg border border-slate-200 text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none w-64"
                />
                ...
                <tbody className="divide-y divide-slate-100">
                    {documents
                        ...
                        >
                            ...
                            <td className="px-6 py-4">
                                {doc.expiry ? (
                                    <span className={`font-mono font-bold ${doc.status === 'Vencido' ? 'text-red-600' : doc.status === 'Por Vencer' ? 'text-amber-600' : 'text-slate-600'}`}>
                                        {doc.expiry}
                                    </span>
                                ) : (
                            ...
                            <td className="px-6 py-4 text-right">
                                <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Descargar">
                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
                                    </button>
                                    <button className="p-2 text-slate-400 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-colors" title="Actualizar">
                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                                    </button>
                                    <button className="p-2 text-slate-400 hover:text-slate-800 hover:bg-slate-100 rounded-lg transition-colors" title="Historial">
