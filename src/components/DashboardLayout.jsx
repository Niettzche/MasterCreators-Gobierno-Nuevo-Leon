...
// PROPÓSITO:
// Layout principal para el área autenticada.
// ADAPTACIÓN: Estilo visual inspirado en DirectoryView.jsx (sidebar de filtros),
// pero con contenido dinámico basado en el rol del usuario (Cluster vs Gobierno).
// Se ha eliminado el Navbar dentro de este layout para usar la vista de Dashboard
// como contenedor principal sin navegación superior duplicada.
// =========================================================
...
export default function DashboardLayout({ children }) {
  ...
  // Estado para controlar qué secciones del menú están abiertas (estilo acordeón)
  ...
  // Componente de Sección de Menú (Estilo DirectoryView FilterSection)
  const MenuSection = ({ title, sectionKey, children }) => (
    <div className="border-b border-gray-100 last:border-0">
      ...
      >
        {title}
        <svg 
          className={`w-4 h-4 text-gray-400 transform transition-transform duration-200 ${openSections[sectionKey] ? 'rotate-180' : ''}`} 
          fill="none" 
          stroke="currentColor" 
          viewBox="0 0 24 24"
        >
  ...
  // Componente de Link Individual (Estilo DirectoryView Options)
    ...
    return (
      <button
        ...
        className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-all flex items-center gap-3 group ${
          active 
            ? 'bg-orange-50 text-orange-700 font-bold shadow-sm ring-1 ring-orange-100' 
      ...
      >
        ...
        {label}
        {active && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-orange-500"></span>}
      </button>
    );
  };

  // Renderizado del Menú según Rol
  const renderSidebarContent = () => {
    if (userRole === 'cluster') {
      return (
        <>
          <MenuSection title="Gestión Operativa" sectionKey="gestion">
            <MenuLink 
              to="/dashboard" 
            ...
            <MenuLink 
              ...
              label="Mis Proyectos" 
              icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 011.414.586l4 4a1 1 0 01.586 1.414V19a2 2 0 01-2 2z"/></svg>} 
            />
            <MenuLink 
              to="/dashboard/evidencias" 
          ...
          <MenuSection title="Administración" sectionKey="finanzas">
            <MenuLink 
      ...
      return (
        <>
          <MenuSection title="Tesorería y Pagos" sectionKey="finanzas_admin">
            <MenuLink
              to="/finance-dashboard"
              label="Dispersión de Recursos"
              icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>}
            />
            <MenuLink
              to="/finance-dashboard/bank-accounts"
      ...
      return (
        <>
          <MenuSection title="Control y Auditoría" sectionKey="auditoria_admin">
            <MenuLink
              to="/audit-dashboard"
            ...
            />
            <MenuLink
              to="/audit-dashboard/reports"
              label="Hallazgos y Reportes"
    ...
    } else if (userRole === 'admin') { // Existing admin logic now under explicit 'admin' role check
      return (
        <>
          <MenuSection title="Supervisión" sectionKey="gestion">
             <MenuLink 
            ...
            />
            <MenuLink 
              to="/admin-dashboard/clusters" 
              label="Padrón de Clústeres" 
              icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg>} 
            />
          </MenuSection>

          <MenuSection title="Programas" sectionKey="admin">
             <MenuLink 
              to="/admin-dashboard/convocatorias" 
  ...
  return (
    <div className="flex h-screen bg-[#F3F4F6] overflow-hidden font-sans">
      ...
      <main className="flex-1 overflow-y-auto relative bg-[#F8FAFC]">
        {/* Top Header Simplificado (Breadcrumbs / Título dinámico) */}
        <header className="sticky top-0 z-20 bg-white/80 backdrop-blur-md border-b border-gray-200 px-8 py-4 flex justify-between items-center">
           <div>
             <h1 className="text-xl font-bold text-slate-800">
               {userRole === 'cluster' ? 'Tablero de Control' : 'Panel de Administración'}
             </h1>
        ...
        {userRole === 'cluster' && (
          <motion.button 
            ...
            onClick={() => navigate('/register')} // Ejemplo: Ir a nueva solicitud
          >
